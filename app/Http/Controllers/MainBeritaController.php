<?php

namespace App\Http\Controllers;

use App\Models\Berita;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Validator;
use Illuminate\Http\UploadedFile;
use Illuminate\Support\Facades\Storage;

use RealRashid\SweetAlert\Facades\Alert;
// or
// use Alert;

class MainBeritaController extends Controller
{
    /**
     * Display a listing of berita.
     *
     * @return \Illuminate\Http\Response
     */
    public function index(){
        // return view('berita');
        $berita = DB::table('berita')->get();
        return view('dashboard.berita.berita',[
            'berita' => $berita]);
    }

    /**
     * Show the form for creating a new berita.
     *
     * @return \Illuminate\Http\Response
     */
    public function add(){
        return view('dashboard.berita.add_berita');
    }

    /**
     * Store a newly created berita in storage.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function store(Request $request){

        $rules = [
			'judul' => 'required|string|max:50|min:5',
			'isi' => 'required|string|max:1024|min:10',
			// 'file' => 'in:xls,xlsx,csv,docx,doc,pdf,txt'
		];
		$validator = Validator::make($request->all(),$rules);

		if ($validator->fails()) {
			return redirect()->route('admin.add_berita')->withInput()->withErrors($validator);
		}
		else{
            $data = $request->input();
            $fileName = 'gaada';
			try{
				$berita = new Berita;
                $berita->judul = $data['judul'];
                $berita->isi = $data['isi'];
                if($request->hasFile('file')){
                    $file = $request->file('file');
                    if ($validExt = Validator::make( [
                            'file'      => $file,
                            'extension' => strtolower($file->getClientOriginalExtension()),
                        ],[
                            'extension'      => 'in:doc,csv,xlsx,xls,docx,ppt,odt,ods,odp,zip,rar',
                        ])->fails()) {
                            return redirect()->route('admin.add_berita')->withInput()->withErrors('Tipe file tidak didukung');
                    }
                    $fileName = time().'_'.$file->getClientOriginalName();
                    $filePath = $file->storeAs('uploads', $fileName, 'public');

                    $berita->nama_file = time().'_'.$file->getClientOriginalName();
                    $berita->file_path = '/storage/app/public/uploads' . $filePath;
                }
				$berita->save();
				return redirect()->route('admin.berita')->with('status', 'Data berhasil ditambahkan');
			}
			catch(Exception $e){
				return redirect()->route('admin.add_berita');
			}
		}
    }

    /**
     * Download berita file from storage.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function downloadFile($file_name){
        // $file = Storage::disk('public')->get($file_name);
        // return (new Response($file, 200))->header('Content-Type', 'application/*');
        $path = storage_path().'/'.'app'.'/public/uploads/'.$file_name;
        return response()->download($path);
    }

    /**
     * Remove the specified resource from storage.
     *
     * @param  \App\Models\book  $book
     * @return \Illuminate\Http\Response
     */
    public function destroy($id){
        DB::delete('delete from berita where id = ?',[$id]);
        // $filename = DB::select('select nama_file from berita where id = ?',[$id]);
        // unlink(storage_path('app/uploads'.$filename));

        // $filename = str_replace('/storage/uploads/', '', $customer->image);
        // // remove old image
        // unlink(storage_path('app/public/uploads/'.$filename));
        return redirect()->route('admin.berita')->with('success','Berita deleted successfully');
    }
}
